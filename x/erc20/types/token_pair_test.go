...
func (suite *TokenPairTestSuite) TestTokenPair() {
	testCases := []struct {
		...
		{msg: "pass", pair: types.TokenPair{utiltx.GenerateAddress().String(), "test", true, types.OWNER_MODULE, "", []string{}}, expectPass: true},
	}
...
func (suite *TokenPairTestSuite) TestIsNativeCoin() {
	testCases := []struct {
		...
		{
			"no owner",
			types.TokenPair{utiltx.GenerateAddress().String(), "test", true, types.OWNER_UNSPECIFIED, "", []string{}},
			false,
		},
		{
			"external ERC20 owner",
			types.TokenPair{utiltx.GenerateAddress().String(), "test", true, types.OWNER_EXTERNAL, "", []string{}},
			false,
		},
		{
			"pass",
			types.TokenPair{utiltx.GenerateAddress().String(), "test", true, types.OWNER_MODULE, "", []string{}},
			true,
		},
	}
